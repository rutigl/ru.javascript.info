...
Что довольно забавно, логика назначения `name` достаточно умная. Она присваивает корректное имя даже в случае, если функция создается без имени и тут же присваивается, например:
...
// здесь невозможно определить имя, поэтому оно отсутствует
```
...
Идея состоит в том, чтобы иметь не только простой синтаксис обработчика без аргументов для положительных ответов (наиболее распространённый случай), но также и возможность передавать универсальные обработчики:
...
Свойство функции, назначенное как `sayHi.counter = 0`, не *объявляет* локальную переменную `counter` внутри неё. Другими словами, свойство `counter` и переменная `let counter` - это две независимые сущности.
...
Для начала заметим, что функция всё ещё задана как Function Expression. Добавление `"func"` после `function` не превращает объявление в Function Declaration, потому что оно всё ещё является частью выражения присваивания.
...
1. Оно позволяет функции ссылаться на саму себя.
...
*!*
    func("Guest"); // использует func, чтобы снова вызвать саму себя
*/!*
  }
};
...
Внешний код всё ещё содержит переменные `sayHi` и `welcome`, но теперь `func` -- это "внутреннее имя функции", таким образом, она может вызвать себя изнутри.
...
- `name` -- имя функции. Обычно берётся из объявления функции, но, если его там нет - JavaScript пытается понять его из контекста.
- `length` -- количество аргументов в объявлении функции. Троеточие ("остальные параметры") не считается.

Если функция объявлена как Function Expression (вне основного потока кода) и имеет имя, тогда это называется Named Function Expression (Именованным Функциональным Выражением). Это имя может быть использовано для ссылки на саму себя, для рекурсивных вызовов и т.п.
